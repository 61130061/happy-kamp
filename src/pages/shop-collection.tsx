...
export default function ShopCollection({
  ...
  useEffect(() => {
    ...
    fetch('https://skillkamp-api.com/v1/api/products/?' + params)
      .then((res) => res.json())
      .then((data) => {
        setShowProducts(
        ...
        <div className="md:flex mb-10">
          <div className="w-full md:w-[180px] lg:w-[240px] p-[20px]">
            <div
              ...
              className="text-2xl border-b pb-[18px]"
            >
              Filter by
            </div>
            {/* Collectionn Filter */}
                ...
                {filters[0].values.map((d, i) => (
                  <div key={i} className="p-1">
                    <input
                      ...
                      type="radio"
                      value={d.value}
                      checked={d.value === filter.CATEGORY}
                      id={d.value}
                      name="category"
                    />
                    <label
                      className="peer-checked:font-semibold py-1 hover:cursor-pointer"
                      htmlFor={d.value}
                    >
                      {d.value}
                    </label>
            ...
            <Accordion title="Price">
              <div className="pt-5 px-2 pb-3">
                ...
                <ReactSlider
                  ...
                  onAfterChange={(s) =>
                    setFilter((prev) => ({ ...prev, PRICE: `${s[0]}-${s[1]}` }))
                  }
            ...
            <Accordion title="Color">
              <div className="flex gap-3 flex-wrap pb-1 pt-3 px-1">
                {filters[2].values.map((d, i) => (
                  <div key={i}>
                    <input
                      onChange={() => onUpdateColor(d.value)}
                      checked={filter.OPTION_COLOR.split(',').includes(d.value)}
                      id={d.key}
                      className="peer sr-only"
                      type="checkbox"
                    />
                    <label
                      htmlFor={d.key}
                      className="w-5 h-5 border-2 border-gray-100 block peer-checked:ring-1 ring-offset-2 ring-primary-1 rounded-full"
                      style={{ backgroundColor: d.key }}
                    />
                  </div>
                ))}
              </div>
            ...
            <Accordion title="Size">
              <div className="text-sm mt-2">
                {filters[3].values.map((d, i) => (
                  <label
                    key={i}
                    className="flex hover:cursor-pointer items-center gap-3 px-2 py-0.5"
                  >
                    <input
                      onChange={() => onUpdateSize(d.value)}
                      checked={filter.OPTION_LIST.split(',').includes(d.value)}
                      type="checkbox"
                    />
                    <div>{d.key}</div>
                  </label>
            ...
            {JSON.stringify(initFilter) !== JSON.stringify(filter) && (
              <div className="text-sm space-y-2 py-[20px]">
                <div>
                  Showing {showProducts.length} of {products.length} products
                </div>
                <button
                  className="flex items-center gap-1 text-primary-4 hover:text-primary-1"
