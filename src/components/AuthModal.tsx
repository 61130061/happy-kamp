...
export default function AuthModal({ isOpen, onClose }: PropsType) {
  ...
  return (
    <div
      ...
      className={`flex items-center justify-center z-[999] top-0 right-0 left-0 bottom-0 fixed h-screen w-screen overflow-hidden bg-white${
        isOpen ? '' : ' hidden'
      }`}
    >
      <button
        name="close-auth-modal"
        onClick={() => onClose()}
...
function Login({ swap }: CompPropsType) {
  ...
  const [form, setForm] = useState<AuthPayloadType>({
    email: '',
    password: '',
  });
  ...
  return (
    <div className="text-center w-[320px]">
      ...
      <div className="mb-5">
        New to this site?{' '}
        <button onClick={() => swap()} className="text-primary-3">
          Sign Up
        </button>
      </div>
      <form onSubmit={onSubmit} className="flex flex-col gap-5 text-left">
        ...
        <button type="button" className="underline text-sm text-left w-fit">
          Forgot password?
        </button>
        <div
          className={`text-sm${success ? ' text-green-500' : ' text-red-500'}`}
        >
          {message}
        </div>
        <button type="submit" className="py-3 my-3 bg-primary-3 text-white">
          Login
        </button>
      </form>
    </div>
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (resData.detail == 'Sign up complete') {
        ...
        setMessage(
          'Sign up complete! Please login with your email and password',
        );
      }
    } catch (err) {
      console.log(err);
  ...
  return (
    <div className="text-center w-[320px]">
      ...
      <form onSubmit={onSubmit} className="flex flex-col gap-5 text-left">
        ...
        <button type="submit" className="py-3 my-5 bg-primary-3 text-white">
          Sign Up
        </button>
      </form>
    </div>
